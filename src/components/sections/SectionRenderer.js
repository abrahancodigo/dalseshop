...
function HeroSection({ config }) {
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setInterval(goNext, autoplaySpeed);
    return () => clearInterval(timeoutRef.current);
  }, [hasMultipleSlides, autoplaySpeed, goNext]);
        ...
        {slides.map((slide, index) => (
          ...
          >
            <div className={styles.slideCard}>
              {slide.image && (
                <img src={slide.image} alt={slide.title || ""} className={styles.heroSlideImg} />
              )}
              <div className={styles.heroOverlay} style={{ opacity: (config.overlayOpacity || 30) / 100 }} />
...
function FeaturedProductsSection({ config }) {
  ...
  useEffect(() => {
    // Fetch a larger batch to ensure enough featured products are found
    getProducts({ isActive: true, limitCount: 50 })
      .then((all) => {
        // Use lenient boolean check
        const featured = all.filter((p) => p.isFeatured === true || p.isFeatured === "true");
        if (featured.length > 0) {
          setProducts(featured.slice(0, limit));
        } else {
          // If no featured, show any active products as fallback
  ...
  return (
    ...
    >
      <div className="container">
        {config.title && <h2 className={styles.sectionTitle}>{config.title}</h2>}
        {config.content && (
          <div className={styles.textContent} dangerouslySetInnerHTML={{ __html: config.content.replace(/\n/g, "<br/>") }} />
        )}
      </div>
    </section>
  );
  ...
  return (
    ...
    >
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        {config.buttonText && (
          <Link href={config.buttonLink || "#"} className={styles.bannerBtn}>
            {config.buttonText}
          </Link>
        )}
      </div>
...
function CustomHtmlSection({ config }) {
  ...
  return (
    <section className={styles.section}>
      <div className="container" dangerouslySetInnerHTML={{ __html: config.code }} />
    </section>
  );
}
