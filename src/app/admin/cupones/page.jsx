...
export default function CuponesPage() {
  ...
  const openCreate = () => { if (!canManage("coupons")) { alert("Usted no tiene los permisos para realizar esta accion"); return; } setEditId(null); setForm(emptyCoupon); setShowModal(true); };
  const openEdit = (c) => { if (!canManage("coupons")) { alert("Usted no tiene los permisos para realizar esta accion"); return; } setEditId(c.id); setForm({ code: c.code || "", type: c.type || "percentage", value: c.value || 0, minPurchase: c.minPurchase || 0, maxUses: c.maxUses || 0, usedCount: c.usedCount || 0, expiresAt: c.expiresAt || "", isActive: c.isActive !== false, description: c.description || "" }); setShowModal(true); };

  const handleSave = async () => {
    if (!canManage("coupons")) { alert("Usted no tiene los permisos para realizar esta accion"); return; }
          ...
          <div className={styles.grid}>
            {coupons.map((c) => (
              <div key={c.id} className={`${styles.card} ${!c.isActive ? styles.inactive : ""}`}>
                ...
                {c.minPurchase > 0 && <span className={styles.couponMin}>Mín: ${formatPrice(c.minPurchase)}</span>}
