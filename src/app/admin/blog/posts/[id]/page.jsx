...
export default function BlogPostEditorPage() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", content: "", excerpt: "", image: "", isPublished: true, author: "Admin", tags: "", category: "" });
  ...
  const loadPost = async () => {
    try {
      ...
      if (post) setForm({ title: post.title || "", slug: post.slug || "", content: post.content || "", excerpt: post.excerpt || "", image: post.image || "", isPublished: post.isPublished || false, author: post.author || "Admin", tags: (post.tags || []).join(", "), category: post.category || "" });
  ...
  return (
    <>
      ...
      <div className={adminStyles.pageContent}>
        <button className="btn btn-ghost btn-sm" onClick={() => navigate("/admin/blog/posts")} style={{ marginBottom: "1rem" }}><HiOutlineArrowLeft /> Volver</button>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "1.5rem", alignItems: "start" }}>
          ...
          <div>
            <div className="admin-card" style={{ marginBottom: "1.5rem" }}>
              ...
              <div className="admin-form-group"><label className="admin-form-label">Categoría</label><input className="admin-form-input" value={form.category} onChange={(e) => handleChange("category", e.target.value)} placeholder="Tecnología, Diseño, etc." /></div>
      ...
      <div className={adminStyles.saveBar}>
        ...
        {canManage("blog") ? (
          ...
          <span className="btn btn-primary btn-lg" style={{ opacity: 0.5, cursor: "not-allowed" }} title="No tienes permisos para guardar artículos">
            Solo lectura
          </span>
        )}
      </div>
