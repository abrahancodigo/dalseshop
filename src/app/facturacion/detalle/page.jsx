...
function DetalleContent() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      doc.save(`Factura_${order.orderNumber || (order.id || "0000").substring(0, 8).toUpperCase()}.pdf`);
    } catch (err) {
      ...
      <main className={styles.main}>
        <div className={`container ${styles.container}`}>
          <div className={styles.topBar}>
            <Link to="/facturacion" className={styles.backLink}>
          ...
          <div className={styles.invoiceCard}>
            <div className={styles.invoiceHeader}>
              ...
              <div className={styles.invoiceMeta}>
                ...
                <p className={styles.invoiceId}>{order.orderNumber ? `#${String(order.orderNumber).padStart(4, "0")}` : `#${(order.id || "0000").substring(0, 8).toUpperCase()}`}</p>
            ...
            <div className={styles.totals}>
              <div className={styles.totalRow}><span>Subtotal</span><span>${formatPrice(order.subtotal)}</span></div>
              {order.discount > 0 && <div className={styles.totalRow}><span>Descuento</span><span>-${formatPrice(order.discount)}</span></div>}
              <div className={styles.totalRow}><span>Envío</span><span>{order.shipping === 0 ? "Gratis" : `$${formatPrice(order.shipping)}`}</span></div>
              <div className={`${styles.totalRow} ${styles.grandTotal}`}><span>Total</span><span>${formatPrice(order.total)}</span></div>
            </div>
